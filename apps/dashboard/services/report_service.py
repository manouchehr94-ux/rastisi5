...
its Order/OrderItem query to it — before this PR, every report figure
(range summary, category breakdown, top products, top customers, and the
90-day weekly chart) queried across every Store's orders unconditionally,
the same bug ``dashboard_service`` had before this same PR fixed it there.
"""
...
def _weekly_sales_chart(store, days: int):
    ...
    rows = (
        Order.objects.filter(store=store, payment_status=Order.PaymentStatus.PAID, created_at__date__gte=start)
...
def sales_chart_for_range(store, range_key: str):
    if range_key == "7":
        return _dashboard_sales_chart_data(store, "week")
    if range_key == "365":
        return _dashboard_sales_chart_data(store, "year")
    if range_key == "90":
        return _weekly_sales_chart(store, RANGE_DAYS["90"])
    return _dashboard_sales_chart_data(store, "month")


def range_summary(store, range_key: str) -> dict:
    days = RANGE_DAYS.get(range_key, 30)
    start = timezone.localdate() - timedelta(days=days - 1)

    orders_in_range = Order.objects.filter(store=store, created_at__date__gte=start)
...
def category_sales_breakdown(store, range_key: str, limit: int = 6):
    days = RANGE_DAYS.get(range_key, 30)
...
def top_products_report(store, range_key: str, limit: int = 6):
    return top_selling_products(store, limit=limit, days=RANGE_DAYS.get(range_key, 30))


def top_customers_report(store, range_key: str, limit: int = 6):
    days = RANGE_DAYS.get(range_key, 30)
    since = timezone.now() - timedelta(days=days)
    return list(
        Order.objects.filter(store=store, created_at__gte=since, payment_status=Order.PaymentStatus.PAID)
...
def build_report_context(store, range_key: str) -> dict:
    from .charts import build_line_chart_svg

    if range_key not in RANGE_DAYS:
        range_key = "30"
    data, labels = sales_chart_for_range(store, range_key)
    return {
        "range_key": range_key,
        "range_filters": RANGE_FILTERS,
        "range_label": RANGE_LABELS[range_key],
        "summary": range_summary(store, range_key),
        "sales_chart_svg": build_line_chart_svg(data, labels, color="#7c3aed"),
        "category_breakdown": category_sales_breakdown(store, range_key),
        "top_products": top_products_report(store, range_key),
        "top_customers": top_customers_report(store, range_key),
    }
