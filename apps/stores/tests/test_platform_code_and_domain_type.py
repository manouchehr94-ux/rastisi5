...
class StoreDomainTypeAndRetirementTests(TestCase):
    def setUp(self):
    ...
    def test_retired_domain_cannot_also_be_primary_via_clean(self):
        domain = StoreDomain(
            store=self.store, hostname="old.example.com", is_primary=True, retired_at=timezone.now(),
        )
        with self.assertRaises(ValidationError):
            domain.clean()

    def test_retired_domain_cannot_also_be_primary_at_db_level(self):
        ...
        # rejects this state.
        domain = StoreDomain.objects.create(
            store=self.store, hostname="old2.example.com", is_primary=False, retired_at=timezone.now(),
    ...
    def test_non_primary_retired_domain_is_allowed(self):
        primary = StoreDomain.objects.create(
            store=self.store, hostname="new.example.com", is_primary=True,
            verification_status=StoreDomain.VerificationStatus.VERIFIED, verified_at=timezone.now(),
        )
        retired = StoreDomain.objects.create(
            store=self.store, hostname="old.example.com", is_primary=False, retired_at=timezone.now(),
            verification_status=StoreDomain.VerificationStatus.VERIFIED, verified_at=timezone.now(),
        )
        self.assertIsNotNone(retired.retired_at)
        self.assertFalse(retired.is_primary)
