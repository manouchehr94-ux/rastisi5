...
تنها مسیرِ تنظیمِ انقضایِ نشست پس از ``auth_login`` در کلِ پلتفرم (مالک،
مشتری، مدیرِ پلتفرم)؛ هیچ ویویی مستقیماً ``request.session.set_expiry``
صدا نمی‌زند (یکپارچه‌سازیِ احرازِ هویت). امنیتِ نشستِ جنگو (چرخشِ کلید پس
از ورود، CSRF، کوکیِ HttpOnly/Secure/SameSite در Production) دست‌نخورده
می‌ماند — این فقط طولِ عمر را تعیین می‌کند، نه مکانیزمِ خودِ نشست را.

عمداً در ``apps.core`` نه ``apps.portal`` — هم مالک و هم مشتری (که در
سلسله‌مراتبِ اپ‌ها زیرِ ``apps.portal`` نیست) باید بدونِ وابستگیِ برعکس
بتوانند از همین یک پیاده‌سازی استفاده کنند."""
