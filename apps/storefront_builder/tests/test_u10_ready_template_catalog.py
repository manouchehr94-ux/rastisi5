...
HISTORICAL_VERSIONS = {
    "dense_marketplace": "2", "premium_leather": "2", "warm_boutique": "2",
    "fashion_promo_catalog": "7", "playful_lifestyle": "1", "utility_catalog": "1",
    "editorial_jewelry": "2", "dark_digital": "2",
}
...
def _historical_preset(key):
    return lpr.get_layout_preset_version(key, HISTORICAL_VERSIONS[key])
...
class RequiredKeysExistTests(TestCase):
    def test_all_eight_required_keys_are_registered(self):
        for key in REQUIRED_KEYS:
            self.assertIsNotNone(_historical_preset(key), key)
...
class MaterialDifferenceTests(TestCase):
    ...
    def test_palettes_are_all_distinct_from_each_other_and_pre_u10_presets(self):
        u10_palettes = [_historical_preset(k).default_palette_slug for k in REQUIRED_KEYS]
    ...
    def test_density_and_motion_are_not_all_identical(self):
        densities = {_historical_preset(k).appearance["density"] for k in REQUIRED_KEYS}
        motions = {_historical_preset(k).appearance["motion"] for k in REQUIRED_KEYS}
    ...
    def test_no_two_templates_have_identical_appearance_dict(self):
        appearances = [tuple(sorted(_historical_preset(k).appearance.items())) for k in REQUIRED_KEYS]
...
class FashionPromoCatalogIsolationTests(TestCase):
    ...
    def test_fashion_promo_catalog_has_two_distinct_category_moments_in_the_right_order(self):
        ...
        preset = _historical_preset("fashion_promo_catalog")
...
class ApplyAndRenderSmokeTests(TestCase):
    ...
    def _apply_and_render(self, key):
        ...
        preset_service.apply_preset(draft, _historical_preset(key))
    ...
    def test_applying_records_correct_provenance(self):
        draft = svc.get_or_create_draft(self.store)
        preset = _historical_preset("editorial_jewelry")
    ...
    def test_reset_to_baseline_works_for_a_u10_template(self):
        draft = svc.get_or_create_draft(self.store)
        preset_service.apply_preset(draft, _historical_preset("playful_lifestyle"))
