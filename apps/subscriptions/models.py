...
class PlatformInvoice(TimeStampedModel):
    """فاکتورِ خریدِ اشتراکِ پلتفرم (Section 8/9) — یک فروشگاه در حالِ پرداختِ
    پلتفرم، نه یک سفارشِ مشتری (که ``apps.orders.Order`` است، دامنه‌ای کاملاً
    جدا). مبلغ در لحظه‌ی ساخت از ``PlanVersion.display_price`` اسنپ‌شات
    می‌شود — تغییرِ بعدیِ قیمتِ پلن هرگز فاکتورِ صادرشده را عوض نمی‌کند.

    تنها راهِ رسیدن به ``PAID``، عبور از یک ``PlatformPaymentAttempt``ی
    موفق است (``billing_service.confirm_payment_attempt``) — هیچ‌جای دیگری
    از کد مستقیماً این وضعیت را نمی‌نویسد."""

    class Status(models.TextChoices):
        PENDING = "pending", "در انتظارِ پرداخت"
        PAID = "paid", "پرداخت‌شده"
        VOID = "void", "باطل‌شده"

    class Reason(models.TextChoices):
        NEW_SUBSCRIPTION = "new_subscription", "خریدِ اشتراکِ تازه"
        UPGRADE = "upgrade", "ارتقاءِ پلن"
        RENEWAL = "renewal", "تمدیدِ اشتراک"

    store = models.ForeignKey(
        "stores.Store", verbose_name="فروشگاه", on_delete=models.PROTECT, related_name="platform_invoices",
    )
    plan_version = models.ForeignKey(
        PlanVersion, verbose_name="نسخه‌ی پلن", on_delete=models.PROTECT, related_name="platform_invoices",
    )
    reason = models.CharField("علت", max_length=20, choices=Reason.choices)
    amount = models.DecimalField("مبلغ (اسنپ‌شات)", max_digits=14, decimal_places=0)
    currency = models.CharField("ارز (اسنپ‌شات)", max_length=8, default="IRT")
    status = models.CharField("وضعیت", max_length=10, choices=Status.choices, default=Status.PENDING, db_index=True)
    public_id = models.CharField("شناسه‌ی عمومی", max_length=32, unique=True, editable=False)
    ...
    created_by = models.ForeignKey(
        ...
        null=True, blank=True, related_name="platform_invoices_created",
    )
    ...
    class Meta:
        ...
        ordering = ["-created_at"]
        indexes = [
            models.Index(fields=["store", "-created_at"], name="idx_platform_inv_store"),
        ]

    def __str__(self):
        return f"Invoice {self.public_id} — {self.store.slug} ({self.get_status_display()})"
...
class PlatformPaymentAttempt(TimeStampedModel):
    """یک تلاشِ پرداختِ مستقل برایِ یک ``PlatformInvoice`` — یک فاکتور می‌تواند
    چند تلاشِ ناموفق داشته باشد پیش از یک تلاشِ موفق (retry)؛ هر تلاش پس از
    رسیدن به وضعیتِ نهایی، تغییرناپذیر می‌ماند (بازپخش/دستکاریِ callback اثر
    نمی‌کند — نگاه کنید به ``billing_service.confirm_payment_attempt``)."""

    class Status(models.TextChoices):
        CREATED = "created", "ایجادشده"
        PENDING = "pending", "در انتظارِ تأییدِ درگاه"
        SUCCEEDED = "succeeded", "موفق"
        FAILED = "failed", "ناموفق"

    FINAL_STATUSES = frozenset({Status.SUCCEEDED, Status.FAILED})

    invoice = models.ForeignKey(
        PlatformInvoice, verbose_name="فاکتور", on_delete=models.PROTECT, related_name="payment_attempts",
    )
    provider_code = models.CharField("کدِ درگاه", max_length=30)
    public_id = models.CharField("شناسه‌ی عمومی", max_length=32, unique=True, editable=False)
    status = models.CharField("وضعیت", max_length=10, choices=Status.choices, default=Status.CREATED)
    provider_ref = models.CharField("ارجاعِ درگاه", max_length=120, blank=True, default="")
    ...
    class Meta:
        ...
        ordering = ["-created_at"]
        indexes = [
            models.Index(fields=["invoice", "-created_at"], name="idx_platform_pay_invoice"),
        ]

    def __str__(self):
        return f"PlatformPaymentAttempt {self.public_id} ({self.get_status_display()})"
