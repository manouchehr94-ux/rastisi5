...
### 2.4 Test coverage — confirmed absent (at the time of the original audit)

No existing test creates section-scoped media, publishes, then calls `get_or_create_draft`/`restore_version`/`apply_industry_layout` again to check whether the media survives. Specifically checked and confirmed **not** to cover this at the time of the original audit:
...
**`[UPDATED — Phase 0.5 checkpoint]` Evidence level: `RUNTIME_VERIFIED`.** This finding was originally reported as `SOURCE_ONLY` (source-traceable, not executed). The owner subsequently reproduced this exact failure mode against a real local database (Store `kianstock-qa`, Published version 21 / Draft version 22, `HeroSlide` id=12 "اسلاید یک" and id=13 "عنوان دومی" both still pointing at `section_id=158` belonging to the archived version 20 — neither the new Published v21 nor the new Draft v22 has those scoped `HeroSlide` placements). Per the owner's explicit instruction, this specific finding is now upgraded to `RUNTIME_VERIFIED` on the owner's authority. Regression tests now exist for this exact scenario (`apps/storefront_builder/tests/test_media_asset_lifecycle.py::HeroSlideCloneTests`), and a fix has been implemented — see `STOREFRONT_BUILDER_V2_PHASE_0_5_REPORT.md`. **The fix itself is not `RUNTIME_VERIFIED`** — it is `SOURCE_ONLY`/`SOURCE_WITH_TEST_COVERAGE` pending the owner's local test execution.
