...
**Category Attribute Schema، Industry Template و UI ویژگی‌های توصیفی کالا (Phase 1E — جدید):**

- فیلدهای پویای ویژگی توصیفی اکنون مستقیماً در فرم ایجاد/ویرایش کالا رندر می‌شوند (طبق طرح ویژگیِ دسته‌بندیِ فعال، با بارگذاری زنده‌ی HTMX هنگام تغییر دسته‌بندی) — نقطه‌ی ناقص نام‌برده‌شده در Phase 1D برطرف شد؛
- `CategoryAttributeSchema` (به‌همراه `is_filterable_override`/`is_comparable_override`/`is_searchable_override` سه‌حالته): نگاشت ویژگی به دسته‌بندی با ارث‌بری تک‌سطحی-به-چندسطحی (نزدیک‌ترین نگاشت همیشه برنده است — ADR-23)، UI مدیریت کامل از صفحه‌ی دسته‌بندی‌ها؛
- سیاست تغییر دسته‌بندی کالا: مقادیر ویژگیِ «یتیم‌شده» هرگز به‌صورت خودکار حذف نمی‌شوند؛ فقط با اقدام صریح مدیر پاک‌سازی می‌شوند (ADR-24)؛
- اعتبارسنجی انتشار (`validate_product_for_publish`): ویژگی‌های الزامیِ طرح دسته‌بندی و قیمت مثبت پیش از `status=active` بررسی می‌شوند؛
- گزینشگر مشخصات/مقایسه‌ی کالا (`build_product_specification`) — منبع واحد داده‌ی گروه‌بندی‌شده برای نمایش مشخصات، بدون فیلد سخت‌کدشده به ازای هر صنف؛
- `IndustryTemplate`/`IndustryTemplateCategory`/`IndustryTemplateAttribute`/... (پلتفرم‌محور، نسخه‌بندی‌شده — ADR-25) و نصب deep-copy یک‌باره روی هر Store (`install_industry_template`، ADR-22) — ۱۰ صنف واقعی seed شده (پوشاک، کفش، عطر، موبایل، رایانه، لوازم خانگی، جواهرات، کتاب، مواد غذایی، مبلمان) از طریق دستور idempotent `seed_industry_templates`؛
- محورهای تنوع پیشنهادیِ هر دسته‌بندی (`CategoryRecommendedOption`) در صفحه‌ی محور تنوع کالا — فقط با تأیید صریح مدیر اعمال می‌شوند، هرگز خودکار.

**چارچوب کیفیت قالب صنف، پیش‌نمایش، و به‌روزرسانیِ امن (Phase 1F — جدید):**

- `IndustryTemplate.readiness` (draft/validation_failed/review_required/production_ready/deprecated/archived) — فقط `production_ready` به مرچنت پیشنهاد یا با آن نصب می‌شود (ADR-26)؛ `install_industry_template` خودش هم این را اجرا می‌کند، نه فقط UI؛
- `template_validation_service.validate_industry_template`: نتیجه‌ی ساختاریافته (خطا/هشدار/اطلاعات با کد پایدار، سنجه‌ها، امتیاز کیفیتِ غیرقابل‌فریب‌توسط‌حجم) + `compute_template_fingerprint` (هش SHA-256 قطعی، مستقل از PK/زمان) + دستور `validate_industry_templates` (`--industry`/`--template-version`/`--json`/`--strict`/`--apply`، فقط‌خواندنی به‌طور پیش‌فرض)؛
- پیش‌نمایش کامل پیش از نصب (`template_preview_service`) و برنامه‌ی اثرِ نصب مشتق از وضعیت واقعیِ Store (نه اعداد ثابت) — صفحه‌ی مستقل در تنظیمات > صنف؛
- مقایسه‌ی نسخه‌ها (`compare_template_versions`) بر اساس کد پایدار (نه برچسب نمایشی) و تشخیص سفارشی‌سازی (`template_customization_service`) با مقایسه‌ی مستقیم با ردیف مبدأ (بدون جدول عکس‌فوری جداگانه — ADR-28)؛
- به‌روزرسانیِ امنِ نصب (`template_update_service`): فقط تغییرات افزودنی (safe_additive) خودکار قابل‌اعمال‌اند؛ تغییرات روی رکورد سفارشی‌شده همیشه مسدود، تغییرات روی رکورد دست‌نخورده نیازمند بازبینیِ صریح‌اند (ADR-29) — اتمیک، idempotent (کلید یکتایی)، تاریخچه‌ی کامل (`StoreTemplateUpdate`)؛
...
- بررسی «کالای دارای تنوع باید حداقل یک تنوع فعال داشته باشد» عمداً از اعتبارسنجی انتشار حذف شده (تناقض با یک تست از پیش موجود و تأییدشده — نگاه کنید به گزارش Phase 1E)؛
- Industry Template فعلاً فقط قابل نصب یک‌بار در طول عمر هر Store است؛ اما اکنون Store می‌تواند نسخه‌ی نصب‌شده را به جدیدترین نسخه‌ی `production_ready` همان خانواده به‌روزرسانی کند (Phase 1F) — تعویض به خانواده‌ی صنف دیگر همچنان پشتیبانی نمی‌شود؛
- اعمال خودکار تغییرات «نیازمند بازبینی» (تغییر نام/نوع‌داده‌ی رکورد دست‌نخورده) در به‌روزرسانی صنف پیاده‌سازی نشده — فقط نمایش داده می‌شود، اعمال آن نیازمند طراحی جداگانه است (ADR-29)؛
- ادغام هوشمند ترکیب‌های منسوخ‌شده هنگام حذف یک محور (سیاست فعلی: فقط منسوخ‌سازی، نه merge خودکار — عمدی، نگاه کنید به ADR-21)؛
- ~~bulk product import/export~~ — پیاده‌سازی شد: صادرات در checkpoint 4 (ADR-49–52) و واردات کالا/تنوع/موجودی از CSV در checkpoint 4B (ADR-55–62)؛
