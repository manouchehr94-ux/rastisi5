...
- **Status:** `ANSWERED`
- **Owner answer:** گزینه A تأیید شد، با تکمیل صریح: هر Family یک Preset و Palette پیش‌فرض خودکار هم دارد. مالک یک سلسله‌مراتب چهارسطحی دقیق‌تر از گزینه‌های اصلی این سؤال تعیین کرد: **Family** (ساختار/DOM/Renderer) → **Preset** (ترکیب آماده *درون* یک Family — چیدمان اولیه Section، Typography، Spacing/Density/Radius/Motion، و یک Palette پیشنهادی؛ هرگز داده فروشگاه را Hard-code نمی‌کند؛ کاملاً قابل‌ویرایش پس از انتخاب) → **Palette** (فقط رنگ؛ کاملاً Global و مستقل از Family/Preset؛ تمام ۲۰ Palette موجود باید برای همه Familyها قابل‌انتخاب بمانند، مشروط به رعایت قرارداد Color Token — هیچ Paletteای فقط به‌خاطر انتخاب Family مخفی/غیرقابل‌انتخاب نمی‌شود؛ ناسازگاری واقعی باید رفع شود، نه با مخفی‌کردن دائمی پوشانده شود) → **Merchant Overrides** (رنگ/فونت/فاصله/تنظیمات مجاز، روی همان مدل موجود «Base Palette + per-token overrides»). تجربه نهایی: `Family → Recommended Preset → Recommended Palette → Optional Global Palette Selection → Merchant Overrides`. علاوه بر این، درباره ۱۰ Template فعلی: نباید به‌عنوان یک انتخاب‌گر ساختاری موازی/مبهم کنار Family نمایش داده شوند؛ هرکدام باید (در فاز طراحی معماری، نه اکنون) به یکی از این‌ها نگاشت شود — یک Legacy Family، یک Preset سازگار درون یک Family، یا یک Alias سازگار با Family+Preset+Palette — بدون اجرای این نگاشت در همین مرحله و بدون تغییر ظاهر فروشگاه‌های موجود؛ باید یک Fallback قطعی برای فروشگاه‌های قدیمی وجود داشته باشد. تغییر Family/Preset/Palette فقط در Draft اثر می‌گذارد (تا Preview+Publish)، هرگز داده تجاری/تنظیمات اختصاصی سایر خانواده‌ها را حذف نمی‌کند (namespaced و بازیابی‌پذیر)، و برای مقادیر مفقود/نامعتبر باید Fallback سازگار با نسخه قبل تعریف شود.
- **Recorded decision:** مکانیزم پیشنهادی سند ۰۸ (بخش ۱ و ۳، پیش‌فرض خودکار قابل‌Override، بدون فیلتر گالری) به‌طور کامل تأیید شد و با یک لایه مفهومی اضافه (**Preset** به‌عنوان یک سطح مستقل و صریح بین Family و Palette، نه صرفاً یک نگاشت ضمنی Family→Template) دقیق‌تر شد. **این تصمیم پیامد مستقیم روی Q-07 دارد** (سؤال «آیا ۵ ورودی جدید به `TEMPLATE_REGISTRY` اضافه شود؟») — چارچوب Q-07 باید بازبینی شود: به‌جای افزودن صرف ۵ ورودی Template، هرکدام از ۵ خانواده به یک Preset پیش‌فرض *جدید* (نه لزوماً معادل یکی از ۱۰ Template موجود) نیاز دارد، و ۱۰ Template موجود جدا و مستقلاً باید به مدل Family/Preset/Alias نگاشت شوند — این بازبینی هنگام رسیدن به Q-07 اعمال می‌شود، نه اکنون. ثبت کامل در `10_OWNER_DECISION_LOG.md`.
