...
- **Status:** `ANSWERED`
- **Owner answer:** گزینه A تأیید شد. بازرسی مستقیم سایت‌های زنده دیگر Blocking نیست؛ بسته `RastiSi_Lightweight_Five_Template_Spec.zip` مرجع اصلی پیاده‌سازی است؛ فایل‌های تصویری آپلودشده (سه ZIP اولیه‌ی این Session: ibolak/cactusleather/deeyarstore) فقط برای درک بهتر زبان بصری/آزمایش قالب‌ها استفاده می‌شوند، نه به‌عنوان شواهد اندازه‌گیری‌شده مستقل.
- **Recorded decision:** بسته سبک، تنها مرجع ساختاری قطعی نسخه اول باقی می‌ماند (دقیقاً همان‌طور که در تمام اسناد ۰۲ تا ۰۷ همین‌طور استفاده شده). این پاسخ همچنین دامنه‌ی کلی فرایند سؤال‌محور را عوض کرد — مالک بلافاصله پس از این پاسخ یک تصمیم جامع (Consolidated Owner Decision) صادر کرد که سؤالات Q-04 تا Q-21 را جمعی حل و فرایند سؤال‌به‌سؤال را متوقف می‌کند؛ به بخش «تصمیم جامع نهایی مالک» در انتهای همین سند و به `10_OWNER_DECISION_LOG.md` مراجعه شود.
...
- **Status:** `RESOLVED BY CONSOLIDATED OWNER DECISION` (۲۰۲۶-۰۸-۰۹) — فرایند سؤال‌به‌سؤال متوقف شد؛ این مورد به‌طور جمعی توسط تصمیم جامع مالک حل شد، نه با پاسخ مجزا. رجوع کنید به بخش «تصمیم جامع نهایی مالک» در انتهای همین سند و به ورودی «تصمیم جامع» در `10_OWNER_DECISION_LOG.md`. توصیه/Default این سند برای این سؤال (بالا) همچنان به‌عنوان مبنای مهندسی معتبر است مگر جایی که تصمیم جامع صریحاً آن را عوض کرده باشد.
- **Owner answer:** (فردی پاسخ داده نشد — بخشی از تصمیم جامع Q-03/تصمیم جامع نهایی)
- **Recorded decision:** (فردی ثبت نشد — به تصمیم جامع نهایی مراجعه شود)
...
## تصمیم جامع نهایی مالک (۲۰۲۶-۰۸-۰۹) — پایان فرایند سؤال‌محور

پس از پاسخ Q-03، مالک یک تصمیم جامع صادر کرد که فرایند سؤال‌به‌سؤال (Q-04 تا Q-21) را متوقف می‌کند و مستقیماً Gate 3 را با عبارت الزام‌شده باز می‌کند: **«START IMPLEMENTATION — the recorded decisions are approved.»** متن کامل تصمیم (۲۰ بند) در `10_OWNER_DECISION_LOG.md` («تصمیم جامع نهایی») ثبت شده است؛ خلاصه‌ی نکاتی که مستقیماً چند سؤال باز را بازنویسی/جایگزین می‌کنند:

- Q-05 (دامنه Category)، Q-06 (Cart/Checkout): بدون تغییر صریح — دامنه هنوز «Home + Product» است؛ تصمیم جامع چیزی درباره Category/Cart اضافه نکرد، پس Default قبلی (خارج از دامنه) پابرجاست.
- Q-07 (۵ ورودی جدید Template): طبق تصمیم جامع بند ۸ و تصمیم Q-02، دیگر «۵ ورودی Template» چارچوب درست نیست — هرکدام از ۵ Family یک **Preset** پیش‌فرض *جدید* می‌گیرد (نه یک `TemplateDefinition` قدیمی)؛ `appearance_registry.py` و ۱۰ Template/۲۰ Palette موجود بدون تغییر مخرب باقی می‌مانند (بند ۷/۸ تصمیم جامع).
- Q-09 (Renderer ثابت یا per-section): بند ۶ تصمیم جامع صراحتاً «Renderer/Partial/Component مخصوص هر Family» + «انتخاب Renderer از طریق Registry/Strategy» را تأیید می‌کند — مطابق Recommendation قبلی این سند (ثابت per-family با استثنای صریح Cactus).
- Q-10 (راهنمای سایز/متادیتای سازنده/کمپین): بند ۱۸ تصمیم جامع («رنگ، سایز، Gallery، ویدئوی محصول، تصاویر Variant، محصولات مرتبط، مشخصات فنی و CTA موبایل باید قابلیت عمومی Builder باشند، نه Hard-coded یک قالب خاص») مسیر اجرا را روشن می‌کند: هر فیلد جدید (اگر لازم شود) باید سطح Builder/عمومی باشد، نه فقط داخل یک Family قفل شود؛ تصمیم دقیق «فیلد جدید بسازیم یا نه» به قضاوت مهندسی زمان اجرا موکول شد (بند ۴ دستورالعمل اجرا: «ابتدا کد موجود را بررسی کن»).
- Q-19 (ترتیب اجرا): تصمیم جامع ترتیب متفاوتی مشخص نکرد؛ ترتیب پیش‌فرض Master Prompt (Modern Fashion → Artisan Editorial → Nordic Living → Heritage Premium → Vibrant Catalog) معتبر می‌ماند.
- Q-20 (سیاست Push/PR): بند ۲۰ تصمیم جامع صریح است — **بدون اجازه صریح مالک هیچ Push/Merge/PR انجام نشود** (دقیقاً همان Default قبلی این سند).

**۱۶ تا ۱۸ بند جدید تصمیم جامع** (پاسخگویی داینامیک واقعی، عدم Hard-code هویت مرجع، حداقل ۱۶px فونت فیلدهای ورودی موبایل، Collection به‌عنوان section مستقل) الزامات *جدیدی* هستند که در هیچ‌کدام از Q-01 تا Q-21 قبلی به این شکل دقیق مطرح نشده بودند — این‌ها مستقیماً به‌عنوان معیار پذیرش Gate 3 اضافه می‌شوند، نه جایگزین یک سؤال قبلی.

**از این نقطه به بعد، طبق دستور صریح مالک، سؤال جدیدی پرسیده نمی‌شود مگر با مانع فنی واقعی و غیرقابل‌حل که ادامه پیاده‌سازی را کاملاً ناممکن کند.**

## تأیید صریح وضعیت

**Gate 2 (تصمیم‌گیری) با تصمیم جامع مالک و عبارت `START IMPLEMENTATION` بسته شد. Gate 3 (پیاده‌سازی) از ۲۰۲۶-۰۸-۰۹ آغاز شده است** — این سند دیگر «قبل از پیاده‌سازی» نیست؛ برای وضعیت واقعی Gate 3 به گزارش‌های پیشرفت جدا مراجعه شود.
