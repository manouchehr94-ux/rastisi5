...
Every confirmed code-level correctness defect (CB-1, the coupon race) is remediated and evidenced under real PostgreSQL row-locking. The private-storage migration-stability gap raised by operator review is corrected and evidenced with a genuine fresh-process test. The Phase-2 product-detail prefetch gap is remediated and evidenced. No new `OPEN_CODE_BLOCKER` was found. What remains open is exactly what the architecture report already named: day-one operational scheduling (cron/systemd for existing management commands) and infrastructure provisioning steps (Redis, object storage) tied to future scale milestones — plus one newly-confirmed, narrowly-scoped, non-blocking N+1 (Section 9) that is a legitimate future performance item, not a launch blocker.
...
Re-verified directly against current source; no reclassification from the architecture report was warranted.

| Item | Current state (verified) | Trigger |
|---|---|---|
| Shared Redis for rate-limit cache | `RASTISI_RATE_LIMIT_CACHE_URL` env-var switch confirmed intact in `shop_core/env_config.py`; `CACHES["rate_limit"]` still `LocMemCache` by default | BEFORE_SECOND_APP_SERVER (precision only; per-process rate limiting still functions correctly today, including on the current 2-Gunicorn-worker production) |
...
| Trusted client IP handling | Not re-verified this session (no evidence gathered) | INSUFFICIENT_EVIDENCE — out of scope for this closure pass; no change since the original Phase 1 hardening |
| Shared rate-limit backend readiness | OPERATOR CONFIGURATION STILL REQUIRED | `RASTISI_RATE_LIMIT_CACHE_URL` unset in this environment; code-side Redis switch confirmed intact (Section 6) |
...
**C. What must still be done before real merchant launch?** Purely operational, not code: schedule the PRE_LAUNCH_REQUIRED commands in Section 5 (`process_notification_outbox`, `expire_inventory_reservations`, `evaluate_subscription_states`, and the billing renewal/dunning pair if paid plans are live) via real cron/systemd. Nothing else is gating launch.

**D. What can safely wait until traffic/benchmark evidence exists?** Everything in Sections 6–8: shared Redis for rate-limiting precision, object-storage provisioning (tied to the *second* app-server instance, not to launch itself), `CONN_MAX_AGE`/PgBouncer, the `Product` composite index, tenant-resolution caching, `pg_trgm` indexing, Gunicorn sizing, and the Section 9 variant-selector N+1 — all correctly deferred, none blocking a pre-market launch with the current single small production instance.
